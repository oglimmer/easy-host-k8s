...
func (s *Store) CreateContent(c *model.Content) error {
	...
	c.UpdatedAt = now
	res, err := s.db.Exec(
		`INSERT INTO content (slug, owner, title, source_url, creator, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		c.Slug, c.Owner, c.Title, c.SourceURL, c.Creator, c.CreatedAt, c.UpdatedAt,
	)
	if err != nil {
		return err
	}
...
func (s *Store) UpdateContent(c *model.Content) error {
	...
	_, err := s.db.Exec(
		`UPDATE content SET title=?, source_url=?, creator=?, updated_at=? WHERE id=?`,
		c.Title, c.SourceURL, c.Creator, c.UpdatedAt, c.ID,
	)
	return err
}
...
func (s *Store) GetContentBySlug(slug string) (*model.Content, error) {
	c := &model.Content{}
	err := s.db.QueryRow(
		`SELECT id, slug, owner, COALESCE(title,''), COALESCE(source_url,''), creator, created_at, updated_at FROM content WHERE slug=?`, slug,
	).Scan(&c.ID, &c.Slug, &c.Owner, &c.Title, &c.SourceURL, &c.Creator, &c.CreatedAt, &c.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return c, nil
...
func (s *Store) ListContentByOwner(owner string, limit, offset int) ([]model.Content, int, error) {
	...
	rows, err := s.db.Query(
		`SELECT id, slug, owner, COALESCE(title,''), COALESCE(source_url,''), creator, created_at, updated_at FROM content WHERE owner=? ORDER BY updated_at DESC LIMIT ? OFFSET ?`, owner, limit, offset,
	)
	if err != nil {
		return nil, 0, err
	}
	...
	for rows.Next() {
		var c model.Content
		if err := rows.Scan(&c.ID, &c.Slug, &c.Owner, &c.Title, &c.SourceURL, &c.Creator, &c.CreatedAt, &c.UpdatedAt); err != nil {
			return nil, 0, err
		}
...
func (s *Store) GetContentFile(slug, filePath string) (*model.ContentFile, error) {
	f := &model.ContentFile{}
	err := s.db.QueryRow(
		`SELECT cf.id, cf.content_id, cf.file_path, cf.file_data, cf.content_type
		 FROM content_file cf JOIN content c ON cf.content_id = c.id
		 WHERE c.slug=? AND cf.file_path=?`, slug, filePath,
	).Scan(&f.ID, &f.ContentID, &f.FilePath, &f.FileData, &f.ContentType)
	if err != nil {
		return nil, err
	}
	return f, nil
